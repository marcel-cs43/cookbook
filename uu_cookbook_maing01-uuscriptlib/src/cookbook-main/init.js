...
class CookbookMainInitClient {
  ...
  async initFinalize() {
    return this.cookbookMainClient.initFinalize(this.lockSecret);
  }
}
/*@@viewOff:scriptClient*/

/*@@viewOn:validateDtoIn*/
const DtoInValidationSchema = `const scriptCookbookMainInitDtoInType = shape({
...
function validateDtoIn(dtoIn, uuAppErrorMap = {}) {
  ...
  let validationResult = dtoInValidator.validate("scriptCookbookMainInitDtoInType", dtoIn);
...
async function main() {
  await console.info(`Script uuCookbook init started`);
  dtoOut.dtoIn = dtoIn;
  ...
  let mainContext = new CookbookMainInitClient(dtoIn.lockSecret);
  ...
  uuCookbook = await mainContext.initFinalize();
